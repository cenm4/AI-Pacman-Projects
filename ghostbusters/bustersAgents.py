    ...
    def chooseAction(self, gameState):
        ...
        for dist in livingGhostPositionDistributions:
            for ghost in dist:
                if dist[ghost] > max:
                    max = dist[ghost]
                    goalCoordinate = ghost

        min = float("inf")
        minAct = 0.0
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            if self.distancer.getDistance(successorPosition, goalCoordinate) < min:
                min = self.distancer.getDistance(successorPosition, goalCoordinate)
                minAct = action
        return minAct
